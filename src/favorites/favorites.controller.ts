import { Controller, Get, Post, Body, Patch, Param, Delete, ParseUUIDPipe, Inject, HttpException, HttpStatus, HttpCode } from '@nestjs/common';
...
export class FavoritesController {
  ...
  createFavTrack(@Param('id', ParseUUIDPipe) id: string) {
    ...
    if (!favTrack) {
      throw new HttpException(`Track with provided id does not exist`, HttpStatus.UNPROCESSABLE_ENTITY);
    }
  ...
  createFavAlbum(@Param('id', ParseUUIDPipe) id: string) {
    ...
    if (!favAlbum) {
      throw new HttpException(`Album with provided id does not exist`, HttpStatus.UNPROCESSABLE_ENTITY);
    }
  ...
  createFavArtist(@Param('id', ParseUUIDPipe) id: string) {
    ...
    if (!favArtist) {
      throw new HttpException(`Artist with provided id does not exist`, HttpStatus.UNPROCESSABLE_ENTITY);
    }
  ...
  getAll(): FavoritesResponseEntity {
    ...
    const favoritesAll = {
      artists: allArtists.filter((artist) => favorites.artists.includes(artist.id)),
  ...
  deleteTrack(@Param('id', ParseUUIDPipe) trackId: string) {
    ...
    if (!favTrackId) {
      throw new HttpException(`Track with provided id is not included in favorites`, HttpStatus.NOT_FOUND);
    }
  ...
  deleteAlbum(@Param('id', ParseUUIDPipe) albumId: string) {
    ...
    if (!favAlbumId) {
      throw new HttpException(`Album with provided id is not included in favorites`, HttpStatus.NOT_FOUND);
    }
  ...
  deleteArtist(@Param('id', ParseUUIDPipe) artistId: string) {
    ...
    if (!favArtistId) {
      throw new HttpException(`Artist with provided id is not included in favorites`, HttpStatus.NOT_FOUND);
    }
